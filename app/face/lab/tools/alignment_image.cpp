...
/*
 * COMMAND:
 *  ./build/tools/alignment_image run_test_on_wflw --input_file=./imgs/t.jpg --model_path=./models/WFLW/WFLW_final/
 */

// DEFINE_int32(thread_num, 1, "thread_num");
DEFINE_string(input_file, "", "input_file");
DEFINE_string(input_file_2, "", "input_file_2");
DEFINE_string(output_file, "", "output_file");
...
//   const int input_width=input_blob[0]->width();
  // const int input_channel=input_blob->channels();
...
//     int thread_id=omp_get_thread_num();
    ...
    timeb t1, t2;
    ftime(&t1);
    Mat image = imread(FLAGS_input_file);
    if(image.data==NULL) {
    ...
    // Copy(cropped_face_1, *(input_blob));
    image.convertTo(image, CV_32F);
    imwrite("./imgs/t_1cim.jpg", image);
    NormalizeImage(image);
    imwrite("./imgs/t_cim.jpg", image);
    Copy(image, *(input_blob));
    deep_align->ForwardPrefilled();
    ...
    printf("===>Preds:\n");
    for (int i = 0; i < landmark_98pt.size(); i++) {
        printf("\t(%f,%f)\n", landmark_98pt[i].x, landmark_98pt[i].y);
    }
